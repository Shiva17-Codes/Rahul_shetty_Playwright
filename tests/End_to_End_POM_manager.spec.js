...
test.skip("End-to-End Testing",async({page})=>{
    ...
    await reg_test.user_reg("Shiva","Singh","shivasingh782@gmail.com","1234567890","Singh@1712","Singh@1712");
    await reg_test.click_login_link();

    await logintest.loginUser1('shiva@gmail123.com','Singh@1712');
    ...
    await checkout_test.enter_detail("1111 1121 1081 1151","1430","SHIVA SINGH","rahulshettyacademy",'11','11');     //Enter the detail of user
    ...
    await checkout_test.searchbox("Ind");      //search the country 
...
//Selecting the product by parameter
test("Click by parameter",async({page})=>{
    ...
    const prod_test=pomanager.getproduct();
    let prod_object={}
    const name_of_prod=['ADIDAS ORIGINAL','ZARA COAT 3'];
    prod_object=await prod_test.select_prod_byparameter(name_of_prod);
    ...
    await cart_test.select_byparameter(prod_object);
    await cart_test.cal_price();                 //calculate the price of the product 
    await cart_test.total_price();                //Total price given by system
